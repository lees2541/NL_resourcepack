...
// 인자로 받아온 값의 정보가 실제 값(ParticleSampler에서 가져온) 것과 매치되는 지 확인, 맞으면 특정 입력을 받았다고 가정한다
...
void main() {
    ...
    switch (int(gl_FragCoord.y)) {
        case 0:
            // Row 0: Time,    첫 쉐이더 작동 후의 시간의 흐름을 인식하게 해준다. 이걸로 애니메이션 연출 이론상 가능
        ...
        case 2:
            ...
            break;
            /*vec4 markerf = texelFetch(ParticleSampler, ivec2(0,2),0);
            // Row 2:
