...
float near = 0.1; 
float far  = 1000.0;
float exposure; // = (int(control_color.b * 255.) == 1 ? 0.4 : 0.6);
float AOE;
...
//const float exposure = 4; //커질수록 더 밝아진다 기존: 2
//const float AOE = 8.; // 커질수록 원의 크기가 작아진다 기존:8
...
void main() {
    ...
    AOE = 10.;
    //float exposure; // = (int(control_color.b * 255.) == 1 ? 0.4 : 0.6);
    //float AOE; //= (int(control_color.b * 255.) == 1 ? 10. : 1.5);
